...
git diff origin/main...HEAD -- src/builder.rs src/checker.rs src/combinators.rs src/capability.rs src/policies/delegating.rs src/session/mod.rs src/results.rs src/facts.rs > mutants.diff
cargo mutants --in-place --in-diff=mutants.diff \
  --file src/builder.rs --file src/checker.rs --file src/combinators.rs \
  --file src/capability.rs --file src/policies/delegating.rs --file src/session/mod.rs \
  --file src/results.rs --file src/facts.rs \
  --timeout=60 --build-timeout=300 --all-features \
  -- --lib --test checker_contract --test tracing_contract \
  --test outcome_contract --test session_contract
```

## CI gates that bite

- Clippy runs with `-D warnings`; any warning fails CI. `fmt` + `clippy` before committing.
- A **diff-scoped `cargo-mutants` gate** covers `src/builder.rs`, `src/checker.rs`, `src/combinators.rs`, `src/capability.rs`, `src/policies/delegating.rs`, `src/session/mod.rs`, `src/results.rs` (decision attribution), and `src/facts.rs` (audit redaction). When changing decisions or short-circuit logic, add a test that distinguishes the mutation (for example, inputs where `&&` and `||` diverge).

...
git diff origin/main...HEAD -- src/builder.rs src/checker.rs src/combinators.rs src/capability.rs src/policies/delegating.rs src/session/mod.rs > mutants.diff
...
- A **diff-scoped `cargo-mutants` gate** covers `src/builder.rs`, `src/checker.rs`, `src/combinators.rs`, `src/capability.rs`, `src/policies/delegating.rs`, and `src/session/mod.rs`. When changing decisions or short-circuit logic, add a test that distinguishes the mutation (for example, inputs where `&&` and `||` diverge).

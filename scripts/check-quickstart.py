...
parser.add_argument("--gatehouse-path", type=pathlib.Path, default=repository)
...
with tempfile.TemporaryDirectory(prefix="gatehouse-quickstart-") as temporary:
    ...
    (consumer / "Cargo.toml").write_text(
        manifest.replace('path = "../../.."', f'path = "{arguments.gatehouse_path.resolve().as_posix()}"')
    )
    subprocess.run(["cargo", "run", "--manifest-path", str(consumer / "Cargo.toml"), "--locked"], check=True)

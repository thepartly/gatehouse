...
- **`checker_batch_one_session / N`** — one session, one batched `filter_authorized_in_session` call. The session deduplicates and the fact source sees one `load_many` covering all `N` keys.

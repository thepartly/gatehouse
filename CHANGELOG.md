...
- Rewrote `examples/rbac_policy.rs` around a domain `Role` enum (exercising the now-generic `RbacPolicy` role type): a compact case table with assertions replaces the print-only walkthrough, output reads as role/document names instead of raw `Uuid`s, and a closing trace explains a denial. Also notes that `RbacPolicy` has no built-in role hierarchy.
- Reframed `examples/policy_builder.rs` so the organization being administered is the *resource* (it previously rode along in `Context`, which the crate docs explicitly advise against): subject is a staff user with scoped grants, the action enum selects the required scope, and `Context = ()`. Contrasts a cross-axis `.when()` predicate with a single-axis `.subjects()` policy and prints a denial trace.
- Trimmed `examples/rebac_policy.rs` to the enum-relation shape only (the duplicate string-relation half is gone; `in_ram_rebac` already covers shared-source production wiring). The case loop asserts every outcome, the fail-closed store-error path is now asserted (`assert_denied` + `assert_trace_contains`), and the trace section points out the session-cache behaviour.
- `examples/lookup_in_ram.rs` now asserts each of its four sections: the authorized listing matches the viewer grants in source order, the admin's lookup-bounded listing is empty, the admin point check grants (via `assert_granted_by`), and the streamed pages agree with the collecting call.
- `examples/combinator_policy.rs` and `examples/mfa_freshness_context.rs` render traces through the new `AccessEvaluation::trace()` accessor instead of hand-rolled matches.
- `examples/postgres_bulk_rebac.rs` passes `&()` directly instead of a `static UNIT_CONTEXT: ()`.
